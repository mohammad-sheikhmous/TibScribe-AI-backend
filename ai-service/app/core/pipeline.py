...
Stage order and why:
  ...
  5. **Clinical canonicalization** — dialect/ASR cleanup with fact-preservation guards.
  6. **Classification** — one batched forward pass over canonical text.
...
class MedicalScribePipeline:
    ...
    def process_with_artifacts(
        ...
        # 5. Dialect/ASR canonicalization. Raw segment text remains immutable; only an
        #    accepted, fact-preserving candidate is allowed downstream.
        ...
        if self.extract_entities:
            for segment in classified:
                segment.entity_links = extract_for_item(
                    {"text": segment.effective_text, "label": segment.label}
                )
    ...
    def _classify(self, segments) -> list[ClassifiedSegment]:
        ...
        """
        texts = [seg.effective_text for seg in segments]
